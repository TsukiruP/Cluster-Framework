...
if(action_state == ACTION_SPIN_DASH) {
    ...
    if(ground == true && input_down == false) {
        if((animation_direction == -1 && !character_collision_left(x, y, angle, mask_big)) || (animation_direction == 1 && !character_collision_right(x, y, angle, mask_big))) {
            x_speed      = animation_direction * (8 + (spin_dash_strength div 2)); 
            action_state = ACTION_ROLL;
            
            // Camera lag
            if(control_type == 1) ctrl_camera.camera_lag = 16;

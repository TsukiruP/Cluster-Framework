...
if (ground == true) {
    ...
    if (on_edge == false && on_obstacle == false) {
        ...
        if (player_line_check(-rela_main_left, main_bottom + 1, true) && !player_line_check(rela_main_right, main_bottom + 1, true)) {
            pos_x = floor(x) - rela_main_left * abs(y_direction);
            pos_y = floor(y) + rela_main_left * x_direction;
        ...
        if (!player_line_check(-rela_main_left, main_bottom + 1, true) && player_line_check(rela_main_right, main_bottom + 1, true)) {
            pos_x = floor(x) + rela_main_right * abs(y_direction);
            pos_y = floor(y) - rela_main_right * x_direction;
...
if (mode == 0 && landed == false && ground_angle == 0) {
    ...
    if (!player_line_check(-rela_main_left, main_bottom + 1, true) && player_line_check(rela_main_right, main_bottom + 1, true)) {
        ...
        // Temporary angle:
        temp_angle = player_get_angle(x - wall_left, y + main_bottom, 0);
    ...
    if (player_line_check(-rela_main_left, main_bottom + 1, true) && !player_line_check(rela_main_right, main_bottom + 1, true)) {
        ...
        // Temporary angle:
        temp_angle = player_get_angle(x + wall_right, y + main_bottom, 0);

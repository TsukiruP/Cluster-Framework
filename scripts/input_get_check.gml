...
with (ctrl_input)
{
    ...
    else if (_device >= DEV_GAMEPAD0)
    {
        ...
        if (_input == INP_ANY)
        {
            return gamepad_get_any(_device, _check) != PAD_NONE;
        }
    ...
    {
        if (_input == INP_ANY) return (input_get_check(_input, _check, DEV_KEYBOARD) || input_get_check(_input, _check, DEV_GAMEPAD0));

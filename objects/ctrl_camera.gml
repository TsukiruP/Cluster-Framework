...
if (instance_exists(stage_get_player(0)))
{
    if (focus_handle == stage_get_player(0))
    {
        ...
        if (look_direction != 0)
        {
            if (camera_look_alarm > 0) camera_look_alarm -= 1;
            ...
            if (camera_look_alarm == 0)
            {
                ...
                {
                    case -1:
                        if (camera_y_shift > -104) camera_y_shift -= 2;
                        break;

                    case 1:
                        if (camera_y_shift < 88) camera_y_shift += 2;
                        break;
                }
            }
        }
        ...
        {
            if (camera_y_shift != 0) camera_y_shift = max(abs(camera_y_shift) - 2, 0) * sign(camera_y_shift);
        ...
        {
            if (camera_x_shift != 64 * focus_handle.image_xscale) camera_x_shift += 2 * focus_handle.image_xscale;
        }
        else
        {
    ...
    if (focus_handle == stage_get_player(0))
    {
        if (camera_position_distance == 0)
        {
            if (camera_lag_alarm == 0)
            {
                ...
                player_offset = 0;
                if (focus_handle.on_ground) player_offset = focus_handle.camera_offset * dsin(focus_handle.mask_rotation);

                if (focus_handle.x < (border_left - player_offset)) camera_x -= min((border_left - player_offset) - focus_handle.x, 24);
                else if (focus_handle.x > (border_right + player_offset)) camera_x += min(focus_handle.x - (border_right + player_offset), 24);
                ...
                if (focus_handle.state_current != player_state_death)
                {
                    ...
                    else if (focus_handle.on_ground)
                    {
                        ...
                        camera_speed_cap = 6;
                        if (focus_handle.x_speed >= 8) camera_speed_cap = 24;

                        if (focus_y < (camera_y - focus_handle.camera_offset)) camera_y -= min((camera_y - focus_handle.camera_offset) - focus_y, camera_speed_cap);
                        else if (focus_y > (camera_y + focus_handle.camera_offset)) camera_y += min(focus_y - (camera_y + focus_handle.camera_offset), camera_speed_cap);
                    }
                }
            }
        }

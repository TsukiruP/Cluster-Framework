...
if (_device == DEV_KEYBOARD) return string_key(config_get_key(_input));
else if (_device >= DEV_GAMEPAD0)
{
    _device -= DEV_GAMEPAD0;
    return string_button(config_get_button(_device, _input), config_get_gamepad(_device, "input_style"));
}
else
{

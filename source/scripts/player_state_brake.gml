...
{
    case STATE_START:
        if ((!config_get("advance_brake") || character_index == CHAR_CLASSIC) && x_speed != 0) image_xscale = sign(x_speed);
    ...
    case STATE_STEP:
            ...
            if (sign(x_speed) != input_x_direction)
            {
                if (input_lock_alarm == 0)
                {
                    ...
                    if (sign(x_speed) == input_x_direction)
                    {
                        ...
                        if (config_get("advance_turn") && character_index != CHAR_CLASSIC && sign(image_xscale) != sign(x_speed))
                        {
        ...
        {
            if (config_get("advance_brake") && character_index != CHAR_CLASSIC) return player_set_state(player_state_run);

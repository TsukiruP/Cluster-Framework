...
var _transition; if (argument_count > 1) _transition = argument[1]; else _transition = game_room_get_transition(_room);
...
with (transition_inst)
{
    ...
    transition_run = game_room_get_run(_room);

    if (!preview)
    {
        if (game_room_get_music() != game_room_get_music(transition_room)) audio_fade_out();

...
global.font_debug = font_add_sprite(fnt_debug, ord("!"), true, 1);
...
var subject_box_alpha_rate; subject_box_alpha_rate = config_get("interface_alpha") / 20;

if (!text_clear && !subject_complete && subject_string != "")
{
    if (subject_box_alpha < config_get("interface_alpha")) subject_box_alpha += subject_box_alpha_rate;
...
var body_box_alpha_rate; body_box_alpha_rate = config_get("interface_alpha") / 20;

if (!text_clear && !ds_list_empty(body_list) && (subject_complete || subject_string == ""))
{
    if (body_box_alpha < config_get("interface_alpha")) body_box_alpha += body_box_alpha_rate;
